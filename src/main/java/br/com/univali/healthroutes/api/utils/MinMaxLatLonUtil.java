package br.com.univali.healthroutes.api.utils;

import lombok.Data;

@Data
public class MinMaxLatLonUtil {
	...
	public MinMaxLatLonUtil(Double lat, Double lon, Double distance) {
//		Double radius = calculateRadius(distance);
//		this.maxLat = lat + radius; // CALCULO DE MAX LATITUDE
//		this.minLat = lat - radius; // CALCULO DE MIN LATITUDE
//		
//		Double deltaLon = calculateDeltaLon(lat, radius);
//		this.maxLon = lon + deltaLon;
//		this.minLon = lon - deltaLon;

//		Double radius = calculateRadius(distance);
		this.maxLat = nextPoint(lat, lon, 0.0, distance)[0]; // CALCULO DE MAX LATITUDE
		this.minLat = nextPoint(lat, lon, 180.0, distance)[0]; // CALCULO DE MIN LATITUDE
		
//		Double deltaLon = calculateDeltaLon(lat, radius);
		this.maxLon = nextPoint(lat, lon, 90.0, distance)[1];
		this.minLon = nextPoint(lat, lon, 270.0, distance)[1];
	}
	
	
//	// FORMULAS
//	private Double calculateRadius(Double distance) {
//		return distance/EARTH_RADIUS;
//	}
//	
//	private Double calculateDeltaLon(Double lat, Double radius) {
//		return Math.asin(Math.sin(radius) / Math.cos(lat));
//	}

	public static Double[] nextPoint(Double latA, Double lonA, Double direcao, Double distancia)
    {
...
//        Double revAz = (Double)Math.atan2((double)sinα, (double)-tmp);
